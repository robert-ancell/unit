...
static void complete_cb(UtObject *object) {
  if (ut_subprocess_get_terminated(process)) {
    printf("Process terminated with signal %d\n",
           ut_subprocess_get_termination_signal(process));
  } else {
    printf("Process returned with status %d\n",
           ut_subprocess_get_exit_status(process));
  }
...
int main(int argc, char **argv) {
  ...
  process = ut_subprocess_new(sleep_argv);
  ...
  ut_subprocess_run(process, dummy_object, complete_cb);
