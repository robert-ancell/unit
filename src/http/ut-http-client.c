...
HttpRequest *http_request_new(const char *host, uint16_t port,
  ...
  ut_list_append_take(headers, ut_http_header_new("Host", host));
  UtObjectRef empty_body = ut_uint8_list_new();
  UtObjectRef body = ut_list_input_stream_new(empty_body);
  request->message_encoder =
      ut_http_message_encoder_new_request(NULL, method, path, headers, body);
