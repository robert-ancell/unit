...
uint32_t *ut_uint32_list_get_writable_data(UtObject *object) {
  UtUint32ListInterface *uint32_list_interface =
      ut_object_get_interface(object, &ut_uint32_list_id);
  assert(uint32_list_interface != NULL);
  if (uint32_list_interface->get_writable_data != NULL) {
    return uint32_list_interface->get_writable_data(object);
  } else {
    return NULL;
  }
}
...
void ut_uint32_list_append(UtObject *object, uint32_t value) {
  ut_uint32_list_append_block(object, &value, 1);
}
...
void ut_uint32_list_prepend(UtObject *object, uint32_t value) {
  ut_uint32_list_prepend_block(object, &value, 1);
}
...
void ut_uint32_list_insert(UtObject *object, size_t index, uint32_t value) {
  ut_uint32_list_insert_block(object, index, &value, 1);
}
