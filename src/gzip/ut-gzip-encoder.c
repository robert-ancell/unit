...
static void write_string(UtGzipEncoder *self, const char *value) {
  for (const char *c = value; *c != '\0'; c++) {
    ut_uint8_list_append(self->buffer, *c);
  }
  ut_uint8_list_append(self->buffer, 0);
}

static void write_header(UtGzipEncoder *self, uint8_t compression_method,
                         bool write_crc, const char *file_name,
                         const char *file_comment, uint32_t modification_time,
  ...
  size_t header_start = ut_list_get_length(self->buffer);
  ut_uint8_list_append(self->buffer, 31);
  ut_uint8_list_append(self->buffer, 139);
  ut_uint8_list_append(self->buffer, compression_method);
  ut_uint8_list_append(self->buffer, flags);
  ut_uint8_list_append_uint32_le(self->buffer, modification_time);
  ut_uint8_list_append(self->buffer, extra_flags);
  ut_uint8_list_append(self->buffer, operating_system);
  if (file_name != NULL) {
    write_string(self, file_name);
  }
  if (file_comment != NULL) {
    write_string(self, file_comment);
  }

  if (write_crc) {
    uint32_t header_crc = 0;
    size_t buffer_length = ut_list_get_length(self->buffer);
    for (size_t i = header_start; i < buffer_length; i++) {
      header_crc =
          crc32(header_crc, ut_uint8_list_get_element(self->buffer, i));
    }
    ut_uint8_list_append_uint16_le(self->buffer, header_crc & 0xffff);
  }
}

static void write_trailer(UtGzipEncoder *self, uint32_t crc,
                          size_t data_length) {
  ut_uint8_list_append_uint32_le(self->buffer, crc);
...
static size_t read_cb(void *user_data, UtObject *data, bool complete) {
  ...
  if (complete) {
    write_trailer(self, self->crc, self->data_length);
  }
...
static UtObjectInterface object_interface = {
    ...
    .init = ut_gzip_encoder_init,
    .cleanup = ut_gzip_encoder_cleanup,
    .interfaces = {{&ut_input_stream_id, &input_stream_interface},
                   {NULL, NULL}}};

UtObject *ut_gzip_encoder_new(UtObject *input_stream) {
  assert(input_stream != NULL);
  UtObject *object = ut_object_new(sizeof(UtGzipEncoder), &object_interface);
  UtGzipEncoder *self = (UtGzipEncoder *)object;
