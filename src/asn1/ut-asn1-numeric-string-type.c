...
static bool valid_numeric_character(char c) {
  return (c >= '0' && c <= '9') || c == ' ';
}
...
static UtObject *ut_asn1_numeric_string_type_check_value(UtObject *object,
                                                         UtObject *value) {
  if (!ut_object_implements_string(value)) {
    return ut_asn1_error_new("Invalid type for NumericString");
  }

  const char *text = ut_string_get_text(value);
  for (const char *c = text; *c != '\0'; c++) {
    if (!valid_numeric_character(*c)) {
      return ut_asn1_error_new("Invalid character in NumericString");
    }
  }
...
static UtAsn1TypeInterface asn1_type_interface = {
    ...
    .check_value = ut_asn1_numeric_string_type_check_value};
