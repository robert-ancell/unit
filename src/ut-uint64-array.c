...
static bool ut_uint64_array_equal(UtObject *object, UtObject *other) {
  UtUint64Array *self = (UtUint64Array *)object;
  if (!ut_object_implements_uint64_list(other)) {
    return false;
  }
  ...
  for (size_t i = 0; i < self->data_length; i++) {
    if (self->data[i] != ut_uint64_list_get_element(other, i)) {
      return false;
    }
  }
  return true;
...
UtObject *ut_uint64_array_new_from_va_elements(size_t length, va_list ap) {
  ...
  resize_list(self, length);
  for (size_t i = 0; i < length; i++) {
    self->data[i] = va_arg(ap, long long);
  }
