...
typedef struct {
  ...
  const uint64_t *(*get_data)(UtObject *object);
  uint64_t *(*get_writable_data)(UtObject *object);
...
/// Use [ut_uint64_list_get_array] if you must have access to the list memory.
...
void ut_uint64_list_append(UtObject *object, uint64_t value);
...
void ut_uint64_list_prepend(UtObject *object, uint64_t value);
