...
static size_t read_data_cb(void *user_data, UtObject *datagram, bool complete) {
...
static size_t read_closed_cb(void *user_data, UtObject *datagram) { return 0; }
...
UtObject *ut_dns_client_new(UtObject *server_address, uint16_t port) {
  ...
  ut_input_stream_read(self->socket, read_data_cb, read_closed_cb, self,
                       self->cancel);
  return object;
}
